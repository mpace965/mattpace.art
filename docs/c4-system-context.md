...
C4Context
    ...
    System_Ext(agent, "Coding Agent", "Writes framework and sketch code, runs tests and linter")
    ...
    Rel(agent, sketchbook, "Writes framework code, runs tests")
...
C4Container
    ...
    Rel(artist, dev_server, "Tunes parameters, views outputs", "Browser")
    Rel(artist, mise, "Runs dev, build, deploy tasks", "CLI")

    Rel(agent, sketches, "Writes pipeline steps")
    Rel(agent, dag_engine, "Writes framework code")
    Rel(agent, github, "Pushes branches, creates PRs")

    Rel(visitor, static_site, "Views published art", "HTTPS")

    Rel(dev_server, dag_engine, "Executes pipelines")
    Rel(bundle_builder, dag_engine, "Executes pipelines")

    Rel(sketches, dag_engine, "Imports base classes", "Python package")
    Rel(static_site, bundle_builder, "Reads manifest and images", "Filesystem")

    Rel(mise, dev_server, "Starts", "uv run dev")
    Rel(mise, bundle_builder, "Runs", "uv run build")
    Rel(mise, static_site, "Builds and deploys", "npm, deploy.sh")
